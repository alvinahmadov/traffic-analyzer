...
cudaError_t cudaYoloLayer_nc(const void *input, void *boxes, void *scores, void *classes, const uint &batchSize,
														 const uint64_t &inputSize, const uint64_t &outputSize, const uint64_t &lastInputSize,
														 const uint &netWidth, const uint &netHeight, const uint &gridSizeX, const uint &gridSizeY,
														 const uint &numOutputClasses, const uint &numBBoxes, const float &scaleXY,
														 const void *anchors, const void *mask, cudaStream_t stream);
...
cudaError_t cudaRegionLayer(const void *input, void *softmax, void *boxes, void *scores, void *classes,
														const uint &batchSize, const uint64_t &inputSize, const uint64_t &outputSize,
														const uint64_t &lastInputSize, const uint &netWidth, const uint &netHeight,
...
YoloLayer::YoloLayer(const void *data, size_t)
{
	...
	for(uint i = 0; i < yoloTensorsSize; ++i)
	{
		...
		read(d, curYoloTensor.grid_size_x);
		read(d, curYoloTensor.grid_size_y);
		read(d, curYoloTensor.num_bboxes);
		read(d, curYoloTensor.scale_xy);
...
size_t YoloLayer::getSerializationSize() const noexcept
{
	...
	for(uint i = 0; i < yoloTensorsSize; ++i)
	{
		...
		totalSize += sizeof(curYoloTensor.grid_size_x);
		totalSize += sizeof(curYoloTensor.grid_size_y);
		totalSize += sizeof(curYoloTensor.num_bboxes);
		totalSize += sizeof(curYoloTensor.scale_xy);
...
void YoloLayer::serialize(void *buffer) const noexcept
{
	...
	{
		const TensorInfo &curYoloTensor = m_YoloTensors.at(i);
		write(d, curYoloTensor.grid_size_x);
		write(d, curYoloTensor.grid_size_y);
		write(d, curYoloTensor.num_bboxes);
		write(d, curYoloTensor.scale_xy);
...
nvinfer1::DimsExprs YoloLayer::getOutputDimensions(INT index, const nvinfer1::DimsExprs *inputs, INT nbInputDims,
																									 nvinfer1::IExprBuilder &exprBuilder) noexcept
{
	...
	if(index == 0)
	{
		return nvinfer1::DimsExprs{
			3, { inputs->d[0], exprBuilder.constant(static_cast<int>(m_OutputSize)), exprBuilder.constant(4) }
		};
	}
	return nvinfer1::DimsExprs{
		3, { inputs->d[0], exprBuilder.constant(static_cast<int>(m_OutputSize)), exprBuilder.constant(1) }
	};
}

bool YoloLayer::supportsFormatCombination(INT pos, const nvinfer1::PluginTensorDesc *inOut, INT nbInputs,
											 ...
											 void const *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
	...
	for(uint i = 0; i < yoloTensorsSize; ++i)
	{
		...
		const uint numBBoxes = curYoloTensor.num_bboxes;
		const float scaleXY = curYoloTensor.scale_xy;
		const uint gridSizeX = curYoloTensor.grid_size_x;
		const uint gridSizeY = curYoloTensor.grid_size_y;
		...
		if(!anchors.empty())
		{
			TADS_CUDA_CHECK(cudaMalloc(&v_anchors, sizeof(float) * anchors.size()))
			TADS_CUDA_CHECK(
					cudaMemcpyAsync(v_anchors, anchors.data(), sizeof(float) * anchors.size(), cudaMemcpyHostToDevice, stream))
		}
		if(!mask.empty())
		{
			TADS_CUDA_CHECK(cudaMalloc(&v_mask, sizeof(int) * mask.size()))
			TADS_CUDA_CHECK(cudaMemcpyAsync(v_mask, mask.data(), sizeof(int) * mask.size(), cudaMemcpyHostToDevice, stream))
		}
		...
		if(!mask.empty())
		{
			if(m_NewCoords)
			{
				TADS_CUDA_CHECK(cudaYoloLayer_nc(inputs[i], boxes, scores, classes, batchSize, inputSize, m_OutputSize,
																		lastInputSize, m_NetWidth, m_NetHeight, gridSizeX, gridSizeY, m_NumClasses,
																		numBBoxes, scaleXY, v_anchors, v_mask, stream))
			}
			else
			{
				TADS_CUDA_CHECK(cudaYoloLayer(inputs[i], boxes, scores, classes, batchSize, inputSize, m_OutputSize, lastInputSize,
																 m_NetWidth, m_NetHeight, gridSizeX, gridSizeY, m_NumClasses, numBBoxes, scaleXY,
		...
		{
			void *softmax;
			TADS_CUDA_CHECK(cudaMalloc(&softmax, sizeof(float) * inputSize * batchSize))
			TADS_CUDA_CHECK(cudaMemsetAsync((float *)softmax, 0, sizeof(float) * inputSize * batchSize, stream))

			TADS_CUDA_CHECK(cudaRegionLayer(inputs[i], softmax, boxes, scores, classes, batchSize, inputSize, m_OutputSize,
																 lastInputSize, m_NetWidth, m_NetHeight, gridSizeX, gridSizeY, m_NumClasses, numBBoxes,
																 v_anchors, stream))

			TADS_CUDA_CHECK(cudaFree(softmax))
		}

		if(!anchors.empty())
		{
			TADS_CUDA_CHECK(cudaFree(v_anchors))
		}
		if(!mask.empty())
		{
			TADS_CUDA_CHECK(cudaFree(v_mask))
		}

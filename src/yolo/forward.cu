...
__global__ void gpuYoloLayer(const float *input, float *boxes, float *scores, float *classes, const uint netWidth,
														 const uint netHeight, const uint gridSizeX, const uint gridSizeY,
														 const uint numOutputClasses, const uint numBBoxes, const uint64_t lastInputSize,
													...
													const void *anchors, const void *mask, cudaStream_t stream)
{
	...
	for(unsigned int batch = 0; batch < batchSize; ++batch)
	{
		gpuYoloLayer<<<number_of_blocks, threads_per_block, 0, stream>>>(
				...
				reinterpret_cast<float *>(classes) + (batch * 1 * outputSize), netWidth, netHeight, gridSizeX, gridSizeY,
				numOutputClasses, numBBoxes, lastInputSize, scaleXY, reinterpret_cast<const float *>(anchors),
